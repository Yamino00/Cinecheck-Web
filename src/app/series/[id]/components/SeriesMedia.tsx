...
interface SeriesMediaProps {
  series: TMDBSeries
}

function getSeriesTrailer(series: TMDBSeries): string | null {
...
export default function SeriesMedia({ series }: SeriesMediaProps) {
  const trailerUrl = getSeriesTrailer(series)
  const [selectedImage, setSelectedImage] = useState<string | null>(null)
  const backdrops = series.images?.backdrops.slice(0, 12) || []
            ...
            {backdrops.map((image, index) => (
              ...
              >
                <Image
                  ...
                  alt={`${series.name} screenshot ${index + 1}`}
                  fill
